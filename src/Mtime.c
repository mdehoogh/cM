	...
	if(NULL==*timezonenames){ // tzn currently not registered, because we ended up on the closing NULL timezone name
		...
		if(_newtimezonenames!=NULL){ // successfully enhanced
			...
			if(timezonename!=NULL){ // managed to create the timezone name to register
				...
				tznindex=0;
				q2outputMessage("Failed to duplicate timezone name '%s'.",tzn);
			}
		}else{
			tznindex=0;
			q2outputMessage(M_ERROR_PREFIX,"Failed to register timezone name '%s'.",tzn);
		}
	}
	return tznindex;
}
	...
	if(tzn!=NULL){
		...
		if(l>2&&tzn[0]!='/'&&tzn[l-1]!='/'&&strrchr(tzn,'/')&&strchr(tzn,'/')!=strchr(tzn,'/')+1){
			...
			if(setenv("TZ",tzn,1)==0){
				...
				q2outputMessage(M_INFO_PREFIX,"System-defined timezone: name='%s' - standard time descriptor='%s' - daylight saving time descriptor='%s' - offset=%li - daylight=%d.\n",tzn,tzname[0],tzname[1],timezone,daylight);
			}else{
				result=1;
				q2outputMessage(M_ERROR_PREFIX,"Failed to register '%s' as system-defined timezone.",tzn);
			}
		}else
			q2outputMessage(M_ERROR_PREFIX,"'%s' is not a valid timezone name.",tzn);
	}
	return result;
}
...
static Mtime* _parsedTime(char const * const iso8601,char const * const tzuser){Mallocationowner owner=getOwner(__LINE__);
	...
	if(l==0){q2outputError("Missing or incorrect ISO8601 timestamp");return NULL;}
	q2outputMessage(M_INFO_PREFIX,"Parsing ISO8601 timestamp '%s'.",iso8601);
	char* Tpos=strchr(iso8601,'T'); // should always be there!!!
	if(NULL==Tpos){q2outputMessage(M_ERROR_PREFIX,"Missing time of day in ISO8601 timestamp '%s'.",iso8601);return NULL;}
	///q2outputMessage(M_INFO_PREFIX,"Parsing ISO8601 timestamp '%s'.",iso8601);
	...
	if(strrchr(iso8601,'+')||strrchr(iso8601,'-')>Tpos){ // NOTE the - of the timezone is BEHIND the position of the 'T'
		...
		if(tzuser!=NULL&&*tzuser){ // explicit user-defined timezone (which might be incorrect!!!!)
			...
			if(tzsys){
				...
				if(NULL==tzenv){q2outputMessage(M_ERROR_PREFIX,"Failed to remember the current timezone '%s'.",tzsys);return NULL;}
				q2outputMessage(M_INFO_PREFIX,"System-defined timezone '%s' remembered.",tzenv);
			}
			// ASSERT tzenv created, so it is essential to free it asap
			q2outputMessage(M_INFO_PREFIX,"Will use the user-defined timezone '%s'.",tzuser);
			...
			if(tznset==0){ // success
				...
				if(strcasecmp(tzuser,"UTC")!=0&&strcasecmp(tzuser,"GMT")!=0&&strcasecmp(tzname[0],"UTC")==0) // apparently now UTC
					q2outputMessage(M_ERROR_PREFIX,"'%s' not considered to be a valid timezone name.",tzuser);
				else
				...
				q2outputMessage(M_ERROR_PREFIX,"Failed to register user-defined timezone '%s'!",tzuser);
			if(tznindex==0)q2outputMessage(M_ERROR_PREFIX,"Failed to remember user-defined timezone '%s'.",tzuser);
		}else
		if(tzsys!=NULL){
			tznindex=_gettznindex(tz=tzsys);
			if(tznindex==0)q2outputMessage(M_ERROR_PREFIX,"Failed to remember system-defined timezone '%s'.",tzsys);
		}else{
			q2outputError("No system or user defined timezone specified!");
			q2outputMessage(M_INFO_PREFIX,"Either add a timezone name to the call, or set the system-defined timezone with a call to settimezone().");
		}
	}
	...
	if(tzsec!=INT16_MIN||tznindex!=0){
		...
		if(month>=1&&month<=12&&monthday>=1&&monthday<=31&&hour>=0&&hour<=23&&minute>=0&&minute<=59&&second>=0&&second<=61){
			...
			if(t>=0){
				...
				if(tz!=NULL)
					q2outputMessage(M_ERROR_PREFIX,"Failed to compute the time represented by '%s' in timezone '%s'.",iso8601,tz);
				else
					q2outputMessage(M_ERROR_PREFIX,"Failed to compute the time represented by '%s'",iso8601);
			}
		}else{
			if(month<1||month>12)q2outputMessage(M_ERROR_PREFIX,"Month (%i) out of range.",month);
			if(monthday<1||monthday>31)q2outputMessage(M_ERROR_PREFIX,"Monthday (%i) out of range.",monthday);
			if(hour<0||hour>23)q2outputMessage(M_ERROR_PREFIX,"Hour (%i) out of range.",hour);
			if(minute<0||minute>59)q2outputMessage(M_ERROR_PREFIX,"Minute (%i) out of range.",minute);
			if(second<0||second>61)q2outputMessage(M_ERROR_PREFIX,"Second (%i) out of range.",second);
		}
	}
	...
	if(tzsec==INT16_MIN){ // no timezone offset defined
		if(tzuser!=NULL&&*tzuser){ // a user-defined timezone was specified, so tzenv should be reinstalled as timezone
			q2outputMessage(M_INFO_PREFIX,"Undoing the registration of user-defined timezone '%s'.",tzuser);
			if(tzenv!=NULL){
				if(setenv("TZ",tzenv,1)!=0)
					q2outputMessage(M_ERROR_PREFIX,"Failed to re-register '%s' as system-defined timezone.",tzenv);
				else
					q2outputMessage(M_INFO_PREFIX,"'%s' re-registered as system-defined timezone.\n",tzenv);
			}else{
				if(unsetenv("TZ")!=0)
					q2outputError("Failed to unregister the system-defined timezone");
				else
					q2outputMessage(M_INFO_PREFIX,"System-defined timezone unregistered!");
			}
			/* replacing:
			int reset=(tzenv?setenv("TZ",tzenv,1):unsetenv("TZ"));
			if(reset!=0)q2outputMessage(M_ERROR_PREFIX,"Failed to %s%s as system-defined timezone.",(tzenv?"restore ":"clear"),(tzenv?tzenv:""));else output("System-defined timezone %s%s.\n",(tzenv?tzenv:""),(tzenv?" restored":"cleared"));
			*/
		}/*else
...
static Mtime* _parsedEpochTime(time_t t,char* tzn){
	...
	if(tzn!=NULL&&*tzn){
		...
		if(tznindex!=0)
			...
			q2outputMessage(M_ERROR_PREFIX,"'%s' not accepted as timezone.",tzn);
	}else
...
Mvalue* Mcalendartime(Mvalue* _timeValue,Mvalue* _tznValue){Mallocationowner owner=getOwner(__LINE__);
	...
	if(_timeValue!=NULL){
		...
		if(_time!=NULL){
				...
				if(tznindex!=0){
					...
					if(Mtimezonenames[tznindex-1]){
						...
						if(_tznset(Mtimezonenames[tznindex-1])!=0){
							q2outputMessage(M_ERROR_PREFIX,"Failed to register '%s' temporarily as system-defined timezone.",Mtimezonenames[tznindex-1]);
							tznindex=INT16_MIN;
						}else
							_tm=localtime(&t);
					}else{
						q2outputMessage(M_BUG_PREFIX,"Timezone name index %d invalid!",tznindex);
			...
			q2outputMessage(M_INFO_PREFIX,"Calendar time fields: year=%d - month=%d - monthday=%d - hour=%d - minute=%d - second=%d.\n"
			...
			if(tznindex!=0){
				if(tzenv!=NULL){
					...
					if(unsetenv("TZ")!=0)
						q2outputMessage(M_ERROR_PREFIX,"Failed to unregister temporary timezone '%s'.",Mtimezonenames[abs(tznindex)-1]);
				}
			}
				...
				if(_iso8601!=NULL){
					...
					if(*t){
							...
							if((*t)&&(month!=INT_MIN)){
								...
								if((c=*t)&&(monthday!=INT_MIN)){
									...
									if(c=='T'||c=='-'||c=='+'){ // there's a timezone part
										q2outputMessage(M_INFO_PREFIX,"Parsing timezone offset '%s'.",t);
										t++;
										t=readint(t,&tzh);
										if((*t)&&(tzh!=INT_MIN)){
											t++;
										...
										q2outputMessage(M_INFO_PREFIX,"Timezone offset parsed!");
									}else
									if(c=='Z'){
										tzh=0;
										q2outputMessage(M_INFO_PREFIX,"Parsing UTC timestamp '%s'.",timetext);
									}
								}
							}
						}
						if(year!=INT_MIN&&month!=INT_MIN&&monthday!=INT_MIN){
							...
							q2outputMessage(M_INFO_PREFIX,"Constructing ISO8601 time string of maximum length %zu.",iso8601characters);
							...
							q2outputMessage(M_INFO_PREFIX,"Parsed to iso8601 text: '%s'.",iso8601);
						}
					}
					/* replacing:
																					...
																					if(string_append_char(_iso8601,'T')){
																							...
																							while(--hmsindex>=0){
																									if(!string_append(_iso8601,"00")||(hmsindex>0&&!string_append_char(_iso8601,':'))){
																											hmsindex=-1;
																											q2outputError("Failed to insert default time defaults");
																									}
																							}
																							if(hmsindex>=-1){
																											...
																											if(string_append_char(_iso8601,timetextcharacter)){
																													...
																													do{
																															...
																															if(digitsleft>0&&!string_insert_char(_iso8601,string_length(_iso8601)-1,'0')){
																																	tzindex=0;
																																	q2outputError("Failed to prepend a trailing zero to the timezone hour.\n");
																															}
																															tzindex--;
																															// append the colon
																															if(tzindex>0&&!string_append_char(_iso8601,':')){
																																	tzindex=-1;
																																	q2outputError("Failed to append the timezone hour - minute colon separator");
																															}
																													}while(tzindex>0);
																													...
																													while(--tzindex>=0){
																															if((tzindex>0||!string_append_char(_iso8601,':'))||!string_append(_iso8601,"00")){
																																	tzindex=-1;
																																	q2outputError("Failed to complete the timezone");
																															}
																													}
																													// if we end up at the end we're complete
																													if(tzindex>=-1)complete=true;
																											}else 
																													q2outputError("Missing hour and/or minute timezone information");
																									}else
																									if(timetextcharacter!='Z'||string_append_char(_iso8601,'Z'))complete=true;
																							}else 
																									q2outputError("Invalid hh(:mm(:ss)) time part");
																					}else 
																							q2outputError("Failed to append the date time separator character T.\n");
																			}else 
																					q2outputError("Invalid monthday");
																	}else 
																			q2outputError("Missing monthday digits");
															}else 
																	q2outputError("Failed to append the month - monthday dash");
													}else 
															q2outputError("Erroneous or incompleted monthday");
											}else
													q2outputError("Missing month digits");
									}else
											q2outputError("Month digits missing");
							}else
									q2outputError("Failed to append the year - month dash");
					}else
							q2outputError("No digits in calendar datetime text");
					*/
					if(complete){
						q2outputMessage(M_INFO_PREFIX,"ISO8501 timestamp: '%s'.",string(_iso8601));
					...
					q2outputError("Failed to initialize the ISO8601 datetime text");
			}else
				q2outputError("Undefined calendar datetime text");
		}else
		if(_timetextValue->type==VT_MAP){
		}else
			q2outputError("Time initializer not an integer, text or map");
	}
	return result;
}
...
Mvalue* Msettimezone(Mvalue* _tznValue){
	if(_tznValue!=NULL&&_tznValue->type==VT_TEXT){
		...
		if(tzn!=NULL&&*tzn){
			...
			/*
			if(tznset<0)
				q2outputMessage(M_ERROR_PREFIX,"'%s' not a valid timezone name.",tzn);
			else
			*/
			if(tznset>0)
				q2outputMessage(M_ERROR_PREFIX,"Failed to register '%s' as (system-defined) timezone.",tzn);
		}else 
			q2outputError("Missing timezone name");
	}else
		q2outputError("Timezone name specified not of type text");

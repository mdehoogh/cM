...
typedef struct Marray{
	...
	Marrayelements* elements; // I suppose we need to have a pointer to an array of Mvalue pointers, alternatively if the number of elements is fixed, we could point to the Mvalue structures themselves?????
	bool weak:1;
	// replacing: bool immutable:1;
}Marray;
...
Marray* _getArray(char* source,unsigned long long numberOfElements,Mvalue const * fillValue);

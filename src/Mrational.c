 ...
 * @details if either is NaN, returns the other
 * @param ld1 
 * @param ld2 
 * @return long double the sum of \p ld1 and \p ld2
 */
static long double ldsum(long double ld1,long double ld2){
	// if either is M_LD_NAN return the other
	if(ld1==M_LD_NAN)return ld2;if(ld2==M_LD_NAN)return ld1;return ld1+ld2;
}
/**
 * @brief returns the sum of M floats \p _float1 and \p _float2
 * @details utilizes ldsum to compute the actual sum
 * @param _float1 
 * @param _float2 
 * @return long double the sum of the long doubles wrapped in M floats \p _float1 and \p _float2
 */
static long double floatsum(Mfloat* _float1,Mfloat* _float2){
...
/**
 * @brief returns a pointer to a new M float containing the sum of M float \p r1 and M float \p r2
 * 
 * @param r1 
 * @param r2 
 * @return Mfloat* the pointer to the sum of the M floats pointed to by \p r1 and \p r2
 */
static Mfloat* _floatsum(Mfloat const * const r1,Mfloat const * const r2){//Mallocationowner owner=getOwner(__LINE__);
...
/**
 * @brief returns the long double difference of long doubles \p ld1 and \p ld2
 * @details if both \p ld1 and \p ld2 are NaN returns NaN; it \p ld1 or \p ld2 is NaN, uses it as if it equals zero
 * @param ld1 
 * @param ld2 
 * @return long double the long double difference of long doubles \p ld1 and \p ld2
 */
// for the computation of the difference of two reals
static long double lddifference(long double ld1,long double ld2){
	// if either is M_LD_NAN return the other
	if(ld1==M_LD_NAN&&ld2==M_LD_NAN)return M_LD_NAN;
	if(ld1==M_LD_NAN)return -ld2;if(ld2==M_LD_NAN)return ld1;return ld1-ld2;
}
/**
 * @brief returns the difference of M floats \p _float1 and \p _float2
 * @details if both are NULL returns NaN; if one is NaN, uses it as if it equals zero
 * @param _float1 
 * @param _float2 
 * @return long double the long double difference of M floats \p _float1 and \p _float2
 */
static long double floatdifference(Mfloat* _float1,Mfloat* _float2){
	if(_float1==NULL&&_float2==NULL)return M_LD_NAN; // both undefined
    // return the other if one is undefined
	if(_float1==NULL)return -_float2->ld; // return the NEGATED value of the second real
	if(_float2==NULL)return _float1->ld;
	// both are defined BUT then we still have the problem that either could be undefined
	return lddifference(_float1->ld,_float2->ld);
}
/**
 * @brief returns the M float difference of M floats \p r1 and \p r2
 * @details if \p r2 is zero or undefined, returns \p r1
 *          if \p r1 is zero or undefined, returns \p -r2
 * @param r1 
 * @param r2 
 * @return Mfloat* the M float representing the difference of M floats \p r1 and \p r2
 */
static Mfloat* _floatdifference(Mfloat* r1,Mfloat* r2){//Mallocationowner owner=getOwner(__LINE__);
	if(floatIsUndefinedOrZero(r2))return _getFloatCopy(r1);
...
/**
 * @brief returns the long double product of long doubles \p ld1 and \p ld2
 * @details if either is NaN, returns NaN
 * @param ld1 
 * @param ld2 
 * @return long double the long double product of \p ld1 and \p ld2
 */
static long double ldproduct(long double ld1,long double ld2){return(isnan(ld1)||isnan(ld2)?M_LD_NAN:ld1*ld2);} // either undefined, product undefined
/**
 * @brief returns the long double product of M floats \p r1 and \p r2
 * @details returns NaN if \p r1 or \p r2 is NULL
 * @param r1 
 * @param r2 
 * @return long double the M float product of \p ld1 and \p ld2
 */
static long double floatproduct(Mfloat const * const r1,Mfloat const * const r2){return(r1!=NULL&&r2!=NULL?ldproduct(r1->ld,r2->ld):M_LD_NAN);} // either undefined, product undefined
/**
 * @brief returns the M float product of the M floats \p r1 and \p r2
 * @details returns NULL if \p r1 or \p r2 is NULL
 * @param r1 
 * @param r2 
 * @return Mfloat* the M float product of \p r1 and \p r2
 */
static Mfloat* _floatproduct(Mfloat const * const r1,Mfloat const * const r2){//Mallocationowner owner=getOwner(__LINE__);
	return(r1!=NULL||r2!=NULL?_getFloat(floatproduct(r1,r2)):NULL);
...
/**
 * @brief returns the long double quotient of long doubles \p ld1 and \p ld2
 * @details return NaN if either is NaN, or (+/-)infinity if the \p ld2 is zero
 * @param ld1 
 * @param ld2 
 * @return long double the quotient of long doubles \p ld1 and \p ld2
 */
static long double ldquotient(long double ld1,long double ld2){
	return(ld1==M_LD_NAN||ld2==M_LD_NAN?M_LD_NAN:(isLongDoubleZero(ld2)==M_TRUE?(signbit(ld1)?M_LD_NEGINF:M_LD_INF):ld1/ld2));
}
// either undefined, quotient undefined
static long double floatquotient(Mfloat* r1,Mfloat* r2){return(r1&&r2?ldquotient(r1->ld,r2->ld):M_LD_NAN);} // either undefined, product undefined
static Mfloat* _floatquotient(Mfloat* r1,Mfloat* r2){//Mallocationowner owner=getOwner(__LINE__);
	return(r1!=NULL||r2!=NULL?_getFloat(floatquotient(r1,r2)):NULL);
} // either undefined, product undefined
// END STATIC FUNCTIONS

// MDH@27FEB2020 replacing: #include "Msession.h"
/**
 * @brief returns the result of multiplying big integer \p a and \p b in big integer \p *c
 * 
 * @param a 
 * @param b 
 * @param _c 
 * @return mp_err MP_OKAY on success, MP_ERR or failure
 */
static mp_err _bimul(Mbiginteger const * const a,Mbiginteger const * const b,Mbiginteger ** _c){//Mallocationowner owner=getOwner(__LINE__);
	...
	if(a!=NULL&&b!=NULL){
		...
		*_c=__biginteger(); // NOTE disowned as we speak 
		return(_c==NULL?MP_ERR:mp_mul(MP_INT_POINTER(a),MP_INT_POINTER(b),MP_INT_POINTER(*_c))); // if both big integers are defined, return the multiplication in *_c
	}
	if(a==NULL&&b==NULL)*_c=NULL;else
	if(a==NULL)*_c=_getBigintegerCopy(b);else
	if(b==NULL)*_c=_getBigintegerCopy(a);
	return MP_OKAY;
	// MDH@02APR2020 END
	/* replacing:
	if(a||b){
...
/**
 * @brief returns the integer divisor of \p a and \p b in big integer \p *_c
 * 
 * @param a 
 * @param b 
 * @param _c 
 * @return mp_err MP_OKAY on success, MP_ERR on failure
 */
static mp_err _bidiv(Mbiginteger const * const a,Mbiginteger const * const b,Mbiginteger ** _c){//Mallocationowner owner=getOwner(__LINE__);
	// MDH@02APR2020: TODO should we do the same here???????
	// assuming _c equals NULL
	if(a!=NULL||b!=NULL){
		*_c=__biginteger();
		if(_c==NULL)return MP_ERR;
		if(a!=NULL&&b!=NULL)return mp_div(MP_INT_POINTER(a),MP_INT_POINTER(b),MP_INT_POINTER(*_c),NULL); // if both big integers are defined, return the divisor in *_c ignoring the remainder!!!
		return mp_copy((a?MP_INT_POINTER(a):MP_INT_POINTER(b)),MP_INT_POINTER(*_c)); // copy either a or b
	}
	// *_c should be NULL, so return MP_ERR if not NULL
	return (*_c!=NULL?MP_OKAY:MP_ERR);
}
/**
 * @brief returns \p a - \b in \p *_c
 * @param a
 * @param b
 * @param _c
 */
static mp_err _bisub(Mbiginteger const * const a,Mbiginteger const * const b,Mbiginteger ** _c){//Mallocationowner owner=getOwner(__LINE__);
	// assuming _c to not be NULL, and *_c to be NULL
	if(a!=NULL&&b!=NULL&&_c!=NULL&&*_c==NULL){
		*_c=__biginteger(); // get a big integer instance
		if(*_c!=NULL)return mp_sub(MP_INT_POINTER(a),MP_INT_POINTER(b),MP_INT_POINTER(*_c)); 
	}
	return MP_ERR;
}
/**
 ...
 * @param _c
 */
static mp_err _biadd(Mbiginteger const * const a,Mbiginteger const * const b,Mbiginteger ** _c){Mallocationowner owner=getOwner(__LINE__);
	// assuming _c to not be NULL, and *_c to be NULL
	if(a!=NULL&&b!=NULL&&_c!=NULL&&*_c==NULL){
		*_c=__biginteger(); // get a big integer instance
		if(*_c!=NULL)return mp_add(MP_INT_POINTER(a),MP_INT_POINTER(b),MP_INT_POINTER(*_c)); 
	}
	return MP_ERR;
}
...
/**
 * @brief places the product of M rationals \p a and \p b in \p c
 * @details \p a, \p b and \p c must not be NULL
 * @param c 
 * @param owner_c 
 * @param a 
 * @param b 
...
mp_err _qmul(Mrational * const c,Mallocationowner owner_c,Mrational const * const a,Mrational const * const b){Mallocationowner owner=getOwner(__LINE__);
	...
	if(a==NULL||b==NULL||c==NULL){
		q2outputError("Cannot compute the product of two rationals: input missing");
		return MP_ERR;
	}
	...
	if(status==MP_OKAY){
		if(_den&&mp_iszero(MP_INT_POINTER(_den))==MP_YES){
			status=MP_ERR;
			q2outputError("The denominator of the product of two rationals is zero");
		}else{
			status=_bimul(a->num,b->num,&_num);owned_biginteger(_num,owner);
			if(status!=MP_OKAY)q2outputError("Failed to compute the product of the rational numerators");
		}
	}else
		q2outputError("Failed to compute the product of the rational denominators");
	/* replacing:
	Mbiginteger *_num=NULL,*_den=NULL;
	...
	*/
	if(status!=MP_OKAY){ // numerator and denominator not computed both
		if(_num!=NULL)FREE_BIGINTEGER(_num,owner);
		if(_den!=NULL)FREE_BIGINTEGER(_den,owner);
...
/**
 * @brief puts the product of M rational \p a and M big integer \p b in M rational \p c
 * 
 * @param c 
 * @param owner_c 
 * @param a 
...
mp_err _qmul_bi(Mrational * const c,Mallocationowner owner_c,Mrational const * const a,Mbiginteger const * const b){Mallocationowner owner=getOwner(__LINE__);
	Mbiginteger *_num=NULL,*_den=NULL;
	mp_err status=(a!=NULL&&b!=NULL&&c!=NULL?MP_OKAY:MP_ERR); // we need all input pointers
...
/**
 * @brief places the result of the division of M rational \p a by M rational \p b in M rational \p c
 * 
 * @param c 
 * @param owner_c 
 * @param a 
...
mp_err _qdiv(Mrational * const c,Mallocationowner owner_c,Mrational const * const a,Mrational const * const b){Mallocationowner owner=getOwner(__LINE__);
	if(a==NULL||b==NULL||c==NULL)return MP_ERR;
	Mbiginteger *_num=NULL,*_den=NULL;
	mp_err status=_bimul(a->den,b->num,&_den); // multiply denominator of a with numerator of a
	owned_biginteger(_den,owner);
	if(status==MP_OKAY)if(_den==NULL||mp_iszero(MP_INT_POINTER(_den))==MP_YES)status=MP_ERR; // and the denominator should be non-zero (division by zero is not possible)
...
/**
 * @brief computes the quotient of M rational \p a and M big integer \p b and puts the result in \p c
 * 
 * @param c 
 * @param owner_c the current owner of \p c
 * @param a 
 * @param b 
 * @return mp_err MP_OKAY on success, MP_ERR on failure
 */
mp_err _qdiv_bi(Mrational * const c,Mallocationowner owner_c,Mrational const * const a,Mbiginteger const * const b){Mallocationowner owner=getOwner(__LINE__);
	if(a==NULL||b==NULL||c==NULL)return MP_ERR;
	Mbiginteger *_num=NULL,*_den=NULL;
	mp_err status=_bimul(a->den,b,&_den); // multiply denominator of a with numerator of a
	owned_biginteger(_den,owner); 
	if(status==MP_OKAY)if(_den==NULL||mp_iszero(MP_INT_POINTER(_den))==MP_YES)status=MP_ERR; // and the denominator should be non-zero (division by zero is not possible)
...
mp_err _qsub(Mrational * const c,Mallocationowner owner_c,Mrational const * const a,Mrational const * const b){Mallocationowner owner=getOwner(__LINE__);
	...
	mp_err status=((c!=NULL&&c->num==NULL&&c->den==NULL)&&(a!=NULL||b!=NULL)?MP_OKAY:MP_ERR); // MDH@24MAY2020: demanding that c->num and c->den are currently undefined!!!
	if(status==MP_OKAY){ // c and at least a or b provided
		...
		if(b==NULL||b->num==NULL||isBigintegerZero(b->num)==M_TRUE){ // b is undefined or zero: return a
			if(report)
				q2outputInfo("Second rational argument undefined or zero.");
			if(a->num!=NULL){c->num=owned_biginteger(_getBigintegerCopy(a->num),Msubowner(owner_c,1));if(c->num==NULL)return MP_ERR;}
			if(a->den!=NULL){c->den=owned_biginteger(_getBigintegerCopy(a->den),Msubowner(owner_c,1));if(c->den==NULL){FREE_BIGINTEGER(c->num,owner);return MP_ERR;}} // MDH@24MAY2020: do NOT forget to free c->num
		}else
		if(a==NULL||a->num==NULL||isBigintegerZero(a->num)==M_TRUE){ // a is undefined or zero: return b
			if(report)
				q2outputInfo("First rational argument undefined or zero.");
			...
			if((a->den==NULL&&b->den==NULL)||(a->den!=NULL&&b->den!=NULL&&(mp_cmp(MP_INT_POINTER(a->den),MP_INT_POINTER(b->den))==MP_EQ))){ // denominators are the same
				if(report)
					q2outputInfo("With equal denominators.");
				...
				if(_num!=NULL){
					...
					if(status==MP_OKAY){
						...
						if(a->den!=NULL){
								...
								if(c->den==NULL){FREE_BIGINTEGER(_num,owner);return MP_ERR;}
						}
						c->num=owned_biginteger(disowned_biginteger(_num,owner),Msubowner(owner_c,1));
					}
				...
				if(report)
					q2outputInfo("With different denominators.");
				Mbiginteger *_num1=NULL,*_num2=NULL,*_den=NULL;
				if(a->den!=NULL||b->den!=NULL){status=_bimul(a->den,b->den,&_den);owned_biginteger(_den,owner);} // we have to be careful here as _bimul requires at least one argument to be non-NULL!!!
				if(status==MP_OKAY)if(_den!=NULL&&mp_iszero(MP_INT_POINTER(_den))==MP_YES)status=MP_ERR; // and the denominator should be non-zero (division by zero is not possible)
				...
				if(_num1!=NULL)FREE_BIGINTEGER(_num1,owner);
				if(_num2!=NULL)FREE_BIGINTEGER(_num2,owner);
				if(status!=MP_OKAY){ // numerator and denominator not computed both
					if(_num!=NULL)FREE_BIGINTEGER(_num,owner);
					if(_den!=NULL)FREE_BIGINTEGER(_den,owner);
		...
		q2outputError("Not all rationals provided for computing the difference of two rationals");
	return status;
}

/**
 * @brief stores the sum of M rationals \p a and \p b in uninitialized M rational \p c
 * 
 * @param c 
 * @param owner_c the current owner of \p c
 * @param a 
 * @param b 
 * @return mp_err MP_OKAY on success, and MP_ERR on failure
 */
mp_err _qadd(Mrational * const c,Mallocationowner owner_c,Mrational const * const a,Mrational const * const b){Mallocationowner owner=getOwner(__LINE__);
	if(c==NULL||c->num!=NULL||c->den!=NULL||c->delta!=NULL)return MP_ERR;
	...
	if(status==MP_OKAY){
		...
		if(status!=MP_OKAY)
			q2outputMessage(M_ERROR_PREFIX,"Failed to multiply the denominators of two rationals (error code: %d).",status);
		else
		if(amVerboseDebugging())
			q2outputInfo("Denominators of two rationals multiplied");
	}
	if(status==MP_OKAY){
		...
		if(_den&&mp_iszero(MP_INT_POINTER(_den))==MP_YES){
			...
			q2outputError("Denominator of the sum of two rationals equal to zero");
		}
	}
	...
	if(status==MP_OKAY){
		...
		if(status!=MP_OKAY)
			q2outputMessage(M_ERROR_PREFIX,"Failed to multiply the numerator and denominator of two rationals (error code: %d).",status);
		else
		if(amVerboseDebugging())
			q2outputInfo("Numerator and denominator of two rationals multiplied.");
	}
	if(status==MP_OKAY){
		status=_bimul(a->den,b->num,&_num2);owned_biginteger(_num2,owner);
		if(status!=MP_OKAY)
			q2outputMessage(M_ERROR_PREFIX,"Failed to multiply the denominator and numerator of two rationals (error code: %d).",status); // multiply denominator of a with numerator of b for the min term of the result numerator
		else
		if(amVerboseDebugging())
			q2outputInfo("Denominator and numerator of two rationals multiplied.");
	}
	if(status==MP_OKAY){
		status=_biadd(_num1,_num2,&_num);owned_biginteger(_num,owner);
		if(status!=MP_OKAY)
			q2outputMessage(M_ERROR_PREFIX,"Failed to add two rational numerators (error code: %d).",status); // add the numerator parts
		else
		if(amVerboseDebugging())
			q2outputInfo("Numerators of two rationals added.");
	}
	// loose the numerator parts (are not stored in the result rational anyway)
	////q2outputInfo("Rational numerator parts to free.");
	if(_num1!=NULL)FREE_BIGINTEGER(_num1,owner);
	if(_num2!=NULL)FREE_BIGINTEGER(_num2,owner);
	////q2outputInfo("Rational numerator parts freed.");
	if(status!=MP_OKAY){ // numerator and denominator not computed both
		q2outputInfo("Freeing new numerator and denominator when failing to compute the sum of two rationals");
		if(_num!=NULL)FREE_BIGINTEGER(_num,owner);
		if(_den!=NULL)FREE_BIGINTEGER(_den,owner);
		//////if(_delta)free_float(_delta);
		////q2outputInfo("New numerator, denominator and delta freed!");
	}else{ // numerator and denominator computed
		////q2outputBiginteger("Storing numerator '",_num,"'");q2outputBiginteger(" and denominator '",_den,"'.\n");
		// too bad we have to clear the current numerator and denominator pointers (if any)
		if(c!=NULL&&c->num!=NULL){if(amVerboseDebugging())q2outputInfo("Freeing previous numerator.");FREE_BIGINTEGER(c->num,owner);}
		if(c!=NULL&&c->den!=NULL){if(amVerboseDebugging())q2outputInfo("Freeing previous denominator.");FREE_BIGINTEGER(c->den,owner);}
		/////q2outputInfo("Previous numerator and denominator released.");
		...
		/////q2outputInfo("Numerator and denominator stored.");
 ...
 * @param a
 * @param b
 * @param owner_b the owner of \p b
 * @result mp_err MP_OKAY on success, and MP_ERR on failure
 */
mp_err _qcopy(Mrational * const b,Mallocationowner owner_b,Mrational const * const a){Mallocationowner owner=getOwner(__LINE__);
	if(b==NULL||b->num!=NULL||b->den!=NULL||b->delta!=NULL)return MP_ERR;
	...
	if(status==MP_OKAY){
		...
		if(status==MP_OKAY){ // copies made successfully
			...
			if(_num!=NULL)FREE_BIGINTEGER(_num,owner);
			if(_den!=NULL)FREE_BIGINTEGER(_den,owner);
			if(_delta!=NULL)FREE_FLOAT(_delta,owner);
		}
	}
	return status;
}
/**
 * @brief copies M big integer \p a into M rational \p b
 * @param a
 * @param b
...
/**
 * @brief returns true if M rational \p q1 equals M rational \p q2
 * 
 * @param q1 
 * @param q2 
 * @param status the cumulative error status
 * @return true 
 * @return false 
 */
bool _qeq(Mrational* q1,Mrational* q2,mp_err *status){Mallocationowner owner=getOwner(__LINE__);
	if(*status!=MP_OKAY)return false;
	if(q1!=NULL&&q2!=NULL){ // both rationals are defined
		...
		*status=_bimul(q1->den,q2->num,&_prod2);owned_biginteger(_prod2,owner);
		...
		if(_prod1!=NULL)FREE_BIGINTEGER(_prod1,owner);
		if(_prod2!=NULL)FREE_BIGINTEGER(_prod2,owner);
		return result;
	}
	return(q1==NULL&&q2==NULL); // if both are NULL return true, false otherwise
}

/**
 * @brief returns the pure rational sum of pure M rationals \p q1 and \p q2
 * 
 * @param q1 a pure M rational
 * @param q2 a pure M rational
 * @return Mrational* the M rational containing the pure sum of \p q1 and \p q2
 */
Mrational* _getPureRationalSum(Mrational const * const q1,Mrational const * const q2){Mallocationowner owner=getOwner(__LINE__);
	...
	if(q1!=NULL&&q2!=NULL){ // rationals defined
		if(floatIsUndefinedOrZero(q1->delta)&&floatIsUndefinedOrZero(q2->delta)){ // both are pure
			...
			if(_pureRationalSum!=NULL){
				if(_qadd(_pureRationalSum,owner,q1,q2)!=MP_OKAY){
					FREE_RATIONAL(_pureRationalSum,owner);_pureRationalSum=NULL;
					// if(amVerbose())
					q2outputError("Failed to compute the sum of two pure rationals");
				}
			}else 
			// if(amVerbose())
				q2outputError("Failed to create the pure sum rational");
		}else 
		// if(amVerbose())
		q2outputError("Both rationals should be pure, and are not");
	}
...
/**
 * @brief returns the true sum of M rationals \p q1 and \p q2
 * 
 * @param q1 
 * @param q2 
 * @return Mrational* the M rational containing the sum of M rationals \p q1 and \p q2
 */
	...
	if(_rational!=NULL){
		...
		if(status==MP_OKAY){
			...
			if(q1->delta!=NULL||q2->delta!=NULL){
				...
				if(!_rational->delta){
					status=MP_ERR;
					q2outputError("Failed to update the delta of the sum of two rationals");
				}
			}
		}else
			q2outputMessage(M_ERROR_PREFIX,"Failed to compute the pure sum of two rationals (error code: %d).",status);
		if(status!=MP_OKAY){
			FREE_RATIONAL(_rational,owner);_rational=NULL;
			// q2outputmessageprefix(M_ERROR_PREFIX);
			// q2outputRational("Failed to compute the sum of rational ",q1,NULL);
			// q2outputRational(" and rational ",q2,".\n");
		}
	}else
		q2outputError("Failed to create the rational for storing the sum of two rationals");
	return disowned_rational(_rational,owner);
}
/**
 * @brief returns the M rational difference of M rationals \p q1 and \p q2
 * 
 * @param q1 
 * @param q2 
 * @return Mrational* the M rational difference of M rationals \p q1 and \p q2
 */
Mrational* _getRationalDifference(Mrational const * const q1,Mrational const * const q2){Mallocationowner owner=getOwner(__LINE__);
	...
	if(_rational!=NULL){
		...
		q2outputError("Failed to create the rational for storing the difference of two rationals");
	return disowned_rational(_rational,owner);
}
/*
...
/**
 * @brief returns the long double product of long double \p ld M rational \p r
 * 
 * @param ld 
 * @param q
 * @return long double the product of long double \p ld and M rational \p q
 */
long double getLongDoubleRationalProduct(long double const ld,Mrational const * const q){
	if(q==NULL)return M_LD_NAN; // unlikely though as we're calling it ourselves with a defined rational
	// TODO there must be other values of ld as well we need to consider like infinity
	long double result=ld;
	if(ld!=M_LD_NAN){
		if(amVerbose())output("Product of long double %.*Lf",LDBL_DIG,ld);
		// multiply ld with the numerator to start with
		if(q->num!=NULL)result=ldproduct(result,mp_get_long_double(q->num));
		// divide ld by the denominator
		if(q->den!=NULL)result=ldquotient(result,mp_get_long_double(q->den));
		if(q->delta!=NULL)result+=ldproduct(ld,q->delta->ld); // MDH@23FEB2023 ADDITION: I think we need to add the product of the M rational delta and ld as well
	}
	if(amVerbose()){q2outputRational(" and rational ",q,":");output("%.*Lf.\n",result);}
	return result;
}
/**
 * @brief returns the long double quotient of long double \p ld and M rational \p q
 * 
 * @param ld 
 * @param q 
 * @return long double the quotient of long double \p ld and M rational \p q
 */
long double getLongDoubleRationalQuotient(long double const ld,Mrational const * const q){
	if(q==NULL)return M_LD_NAN; // unlikely though as we're calling it ourselves with a defined rational
	long double result=ld;
	// TODO check if the following computation is correct!!!!
	if(ld!=M_LD_NAN){ // TODO consider other special values of ld as well
		// multiply ld with the numerator to start with
		// MDH@23FEB2023: the denominator is delta*denominator+numerator
		long double ld_den=(q->den!=NULL?mp_get_long_double(q->den):1.0);
		if(q->den!=NULL)result=ldproduct(result,ld_den); // no need to multiply by 1 is it
		// divide ld by the denominator
		long double dividend=(q->num!=NULL?mp_get_long_double(q->num):1.0);
		if(q->delta!=NULL)dividend+=ldproduct(q->delta->ld,ld_den); // add the product of delta times the denominator of q
		if(dividend!=1.0)result=ldquotient(result,dividend);
	}
	return result;
}
/**
 * @brief returns the long double sum of long double \p ld and M rational \p q
 * 
 * @param ld 
 * @param q 
 * @return long double the sum of long double \p ld and M rational \p q
 */
long double getLongDoubleRationalSum(long double const ld,Mrational const * const q){
	if(q==NULL)return M_LD_NAN; // unlikely though as we're calling it ourselves with a defined rational
	long double result=ld;
	if(ld!=M_LD_NAN){ // TODO address other special values as well
		// multiply ld with the numerator to start with
		if(q->num!=NULL)result=ldproduct(result,mp_get_long_double(q->num));
		// divide ld by the denominator
		if(q->den!=NULL)result=ldquotient(result,mp_get_long_double(q->den));
		if(q->delta!=NULL)result=ldsum(result,q->delta->ld); // MDH@23FEB2023: do not forget to add the delta as well
	}
	return result;
}

/**
 * @brief returns the product of the two rationals \p q1 and \p q2
 * 
 * @param q1 
 * @param q2 
 * @return Mrational* the product of rationals \p q1 and \p q2
 */
	...
	if(_rational!=NULL){
		...
		if(status==MP_OKAY){
			...
			if(delta1defined||delta2defined){
				...
				long double term1=(delta1defined?getLongDoubleRationalProduct(q1->delta->ld,q2):M_LD_NAN);
				long double term2=(delta2defined?getLongDoubleRationalProduct(q2->delta->ld,q1):M_LD_NAN);
				...
				if(q1->delta!=NULL&&q2->delta!=NULL)if(_rational->delta==NULL){
					status=MP_ERR;
					q2outputmessageprefix(M_ERROR_PREFIX);
					q2outputRational("Failed to update the delta of the product of rational ",q1,NULL);
					q2outputRational(" and ",q2,".\n");
				}
			}
		}else{
			q2outputmessageprefix(M_ERROR_PREFIX);
			q2outputRational("Failed to multiply rational ",q1,NULL);
			q2outputRational(" and ",q2,".\n");
		}
		if(status!=MP_OKAY){FREE_RATIONAL(_rational,owner);_rational=NULL;}
	}else
		q2outputError("Failed to create the rational for storing the product of two rationals");
	return disowned_rational(_rational,owner);
}
/**
 * @brief returns the quotient of rationals \p q1 and \p q2
 * 
 * @param q1 
 * @param q2 
 * @return Mrational* the quotient of rationals \p q1 and \p q2
 */
Mrational* _getRationalQuotient(Mrational const * const q1,Mrational const * const q2){Mallocationowner owner=getOwner(__LINE__);
	...
	if(_rational!=NULL){
		...
		q2outputError("Failed to create the rational for storing the quotient of two rationals.");
	return disowned_rational(_rational,owner);
}
// MDH@05NOV2019: if we want to divide a rational by a big integer
/**
 * @brief returns the quotient of rational \p q and big integer \p b
 * 
 * @param q 
 * @param b 
 * @return Mrational* the quotient of rational \p q and big integer \p b
 */
Mrational* _getRationalBigintegerQuotient(Mrational const * const q,Mbiginteger const * const b){Mallocationowner owner=getOwner(__LINE__);
	...
	if(b!=NULL&&!isBigintegerZero(b)){
		...
		if(_newnum!=NULL){
			...
			q2outputError("Failed to copy a big integer");
	}
...
/**
 * @brief returns the sine/cosine function of the rational \p x 
 * @details uses the ordinary Taylor polynomial approximation
 * @param x the argument of which to compute the rational approximation to the sine/cosine
 * @param sin when true, the sine of \p x is returned, otherwise the cosine of \p x
 */
Mrational* _qsinorcos(Mrational const * const x,bool sin){Mallocationowner owner=getOwner(__LINE__);
	...
	if(x!=NULL){
		...
		if(_sinorcos&&_prevsincos&&_x2&&_x4&&_multnum&&_multden&&_mult&&_den&&_4n&&_prod&&_1minus&&_sub&&_1&&_prodacc&&_prevprodacc){
			...
			while(status==MP_OKAY){
				...
				if(!isRationalZero(_prevprodacc)){ // accuracy not yet reached
					...
					if(_intermediateResult){
						...
						if(istatus==MP_OKAY){
							q2outputRational("Increment: '",_intermediateResult,"' -> ");
							if(sin)istatus=_qmul(_intermediateResult,owner,_sinorcos,x);else istatus=_qcopy(_intermediateResult,owner,_sinorcos);
							if(istatus==MP_OKAY)q2outputRational((sin?"Sine: ":"Cosine: '"),_intermediateResult,"'.\n");
						}
					}
					...
					if(_intermediateResult){
						...
						if(istatus==MP_OKAY)q2outputRational("Incremental remainder: '",_intermediateResult,"'.\n");
					}
			...
			q2outputmessageprefix(M_ERROR_PREFIX);
			q2output("Failed to initialize the result of computing the %ssine",(sin?"":"co"));
			q2outputRational(" of '",x,"'.\n");
		}
...
/**
 * @brief constructs the rational present in the text representation of a decimal stored in \p decimalText
 * @param decimalText 
 * @return the rational represented in \p decimalText
 */
		...
		if(exponentText!=NULL){
			*exponentText='\0'; // 'cuf off' the e-part!!!!
			...
			if(_exponent!=NULL&&(mp_read_radix(MP_INT_POINTER(_exponent),exponentText,10)!=MP_OKAY)){
				q2outputMessage(M_ERROR_PREFIX,"Failed to extract the exponent from its text representation '%s'.\n",exponentText);
				FREE_BIGINTEGER(_exponent,owner);
				_exponent=NULL;
			}else
				if(_exponent!=NULL)
					if(amVerboseDebugging())
						q2outputBiginteger("Exponent '",_exponent,"'.\n");
		}
		if(exponentText==NULL||_exponent!=NULL){ // either we do not have an exponentText or we have an exponent big integer (to apply later on)
			...
			if(decimalPartText!=NULL)*decimalPartText='\0'; // 'cut off' the decimal part (for now)
			...
			if(_numerator&&(mp_read_radix(MP_INT_POINTER(_numerator),decimalText,10)==MP_OKAY)){ // apparently a valid (big) integer
				...
				if(decimalPartText){
					...
					if(_decimalPartBiginteger&&(mp_read_radix(MP_INT_POINTER(_decimalPartBiginteger),decimalPartText,10)==MP_OKAY)){
						if(isBigintegerZero(_decimalPartBiginteger)!=M_TRUE){
							...
							if(amVerboseDebugging())
								if(_denominator!=NULL)q2outputBiginteger("Denominator: '",_denominator,"'.\n");
						...
						if(amVerboseDebugging())
							q2outputBiginteger("Decimal part integer: '",_decimalPartBiginteger,"'.\n");
					}
				}
				// if we have a decimalPartText we need a denominator
				if(decimalPartText==NULL||_denominator!=NULL){
					...
					if(!_denominator
						...
						if(amVerboseDebugging())
						{
							q2outputBiginteger("Numerator before applying the exponent: '",_numerator,"'.\n");
							if(_denominator!=NULL)q2outputBiginteger("Denominator before applying the exponent: '",_denominator,"'.\n");
						}
				...
				q2outputMessage(M_ERROR_PREFIX,"Integer part of rational text '%s' invalid.\n",decimalText);
...
/**
 * @brief sets the owner of rational \p _rational to \p owner_rational
 * 
 * @param _rational 
 * @param owner_rational 
 * @return Mrational* the newly owned rational \p _rational
 */
...
/**
 * @brief disowns rational \p _rational currently owned by \p owner_rational
 * 
 * @param _rational 
 * @param owner_rational 
 * @return Mrational* the disowned rational \p _rational
 */
...
/**
 * @brief frees rational \p _rational
 * @param _rational the rational to free
 */
...
/**
 * @brief normalizes rational \p _rational
 * 
 * @param _rational the rational to normalize
 * @param owner_rational the current owner of \p _rational
 * @return true on success
 * @return false on failure
 */
bool normalizeRational(Mrational * const _rational,Mallocationowner owner_rational){if(_rational==NULL||_rational->normalized)return true;Mallocationowner owner=getOwner(__LINE__);
		...
		if(_gcd){
			...
			if(!isBigintegerOne(_gcd)){
				if(mp_gcd(MP_INT_POINTER(_rational->num),MP_INT_POINTER(_rational->den),MP_INT_POINTER(_gcd))==MP_OKAY){
					if(mp_cmp(MP_INT_POINTER(_gcd),MP_INT_POINTER(getBigintegerOne()))!=MP_EQ){ // equal to 1 apparently no need to divide num and den by the gcd and then consider normalized
						...
						if(_newnum&&_newden&&mp_div(MP_INT_POINTER(_rational->num),MP_INT_POINTER(_gcd),MP_INT_POINTER(_newnum),NULL)==MP_OKAY
							...
							_rational->normalized=true;
								...
								q2outputError("Failed to compute the new numerator and denominator");
					...
					q2outputError("Failed to compute the GCD");
			}else // GCD=1, already normalized apparently
				_rational->normalized=true;
			FREE_BIGINTEGER(_gcd,owner); // OOPS essential!!
		}else
			q2outputError("Failed to create the GCD");
	}else{
		_rational->normalized=true;
		q2outputWarning("Normalized flag of rational not set although the denominator equals 1; flag set");
	}
...
/**
 * @brief returns a new rational
 * 
 * @return Mrational* the created rational
 */
Mrational* __rational(){Mallocationowner owner=getOwner(__LINE__);
	...
	if(_rational==NULL)outputMemoryError("Failed to create a rational");
	if(_rational->num!=NULL||_rational->den!=NULL||_rational->delta!=NULL)q2outputBug("New rational numerator, denominator and delta not all considered undefined");
	return disowned_rational(_rational,owner);
}/* VALIDATED */
...
/**
 * @brief returns a new rational with numerator \p _numerator and denominator \p _denominator and delta \p delta
 * @details if \p normalize is true, the rational returned will be normalized
 * @param _numerator
 * @param _denominator
 * @param delta
 * @param normalize flag
 * @return Mrational* the newly created rational
 */
Mrational* _getRational(Mbiginteger const * const _numerator,Mbiginteger const * const _denominator,long double delta,bool normalize/*,bool freeifunbound*/){Mallocationowner owner=getOwner(__LINE__);
	...
	if(_denominator!=NULL)if(isBigintegerZero(_denominator)==M_TRUE)return NULL; // MDH@25MAY2020: denominator can't be zero!!
	if(amVerboseDebugging())
	{
		q2outputBiginteger("Determining the rational with numerator ",_numerator,NULL);
		q2outputBiginteger(" and denominator ",_denominator,".\n");
	}
	Mrational* _rational=owned_rational(__rational(),owner);
	if(_rational!=NULL){
		...
		if(!isLongDoubleUndefined(delta)){ // we need a delta
			...
			if(_rational->delta==NULL){FREE_RATIONAL(_rational,owner);_rational=NULL;q2outputError("Failed to create the rational delta");}
		}
		if(_rational!=NULL){
			...
			if(_nonnullnumerator!=NULL){
				...
				if(_denominator!=NULL&&mp_isneg(MP_INT_POINTER(_denominator))==MP_YES){ // the given denominator is negative             
					if(amVerboseDebugging())q2outputInfo("Moving the sign from the denominator to the numerator of the rational.");
					...
					if(_negatedNumerator!=NULL&&_negatedDenominator!=NULL){ // succeeded in negating the numerator and denominator
						if(amVerbose())q2outputInfo("Using the negated numerator and denominator.");
						...
						*/
						...
						q2outputError("Failed to move the sign from the denominator to the numerator of a rational");
					}
					// essential to free any numerator we created ourselves!!!!
					if(_numerator==NULL)FREE_BIGINTEGER(_nonnullnumerator,owner);
				}else{
					_rational->num=SUBOWNED(_nonnullnumerator,1); // could be NULL now when it's the inverse of another rational
					if(_denominator!=NULL)_rational->den=owned_biginteger(_getBigintegerCopy(_denominator),Msubowner(owner,1)); // MDH@26MAY2020: copying the denominator if it is not NULL
				}
				...
				FREE_RATIONAL(_rational,owner);_rational=NULL;
				q2outputError(_numerator!=NULL?"Undefined rational numerator":"Failed to create big integer 1");
			}
		}
	}else
		q2outputError("Failed to create a rational");
...
/**
 * @brief returns the inverse of rational \p _rational
 * 
 * @param _rational 
 * @return Mrational* the inserver of \p rational
 */
Mrational* _getInverseRational(Mrational const * const _rational){Mallocationowner owner=getOwner(__LINE__);
	if(_rational==NULL){q2outputError("No rational to invert");return NULL;}
	...
	if(isFloatUndefined(_rational->delta)==M_TRUE||isFloatZero(_rational->delta)==M_TRUE){
		...
		if(_inverseRational==NULL){
			q2outputError("Failed to create the inverse rational");
			return NULL;
		}
		if(_rational->normalized)_inverseRational->normalized=true; // nasty TODO check if this is correct
	}else // TODO we'd like to implement this sometime
		q2outputError("Can't invert an unpure rational");
...
/**
 * @brief returns the text representation of rational \p _rational
 * 
 * @param _rational 
...
/**
 * @brief outputs rational \p _rational prefixed by \p prefix and suffixed by \p suffix
 * 
 * @param prefix 
 * @param _rational 
 * @param postfix 
 ...
 */
size_t outputRational(const char* const prefix,const Mrational* const _rational,const char* const suffix){Mallocationowner owner=getOwner(__LINE__);
	size_t written=(prefix!=NULL?output("%s",prefix):0);
	if(_rational!=NULL){
		Mstring* _rationalText=owned_string(_getRationalText(_rational),owner);
		if(_rationalText){
			written+=output("%s",string(_rationalText));
 ...
 * @return the number of characters output
 */
size_t q2outputRational(const char* const prefix,const Mrational* const _rational,const char* const suffix){Mallocationowner owner=getOwner(__LINE__);
	...
	if(_rational!=NULL){
		...
		if(_rationalText){
			...
			q2output("%s","no rational text representation");
	}else
		written+=q2output("%c",'?');
...
/**
 * @brief returns the big integer part of rational \p _rational
 * @details truncates the rational towards zero
 * @param _rational 
...
Mbiginteger* _rational2biginteger(Mrational* _rational){Mallocationowner owner=getOwner(__LINE__);
	...
	if(_biginteger==NULL){q2outputError("Failed to create a big integer");return NULL;}
	if(_rational->den!=NULL){
		...
		if(_rational->num!=NULL&&isBigintegerZero(_rational->num)!=M_TRUE){
			...
			if(neg){
				...
				if(absnum!=NULL){
					if(mp_neg(MP_INT_POINTER(_rational->num),MP_INT_POINTER(absnum))!=MP_OKAY){
						...
						q2outputError("Failed to negate a numerator truncating a rational");
					}else
						q2outputError("Failed to create a big integer");
				}
			}else 
			...
			if(absnum!=NULL){
				if(mp_div(MP_INT_POINTER(absnum),MP_INT_POINTER(_rational->den),MP_INT_POINTER(_biginteger),NULL)!=MP_OKAY){
					...
					q2outputmessageprefix(M_ERROR_PREFIX);
					q2outputBiginteger("Failed to divide numerator ",absnum," ");
					q2outputBiginteger("by denominator ",_rational->den,".\n");
				}else
				if(neg&&mp_neg(MP_INT_POINTER(_biginteger),MP_INT_POINTER(_biginteger))!=MP_OKAY){
					FREE_BIGINTEGER(_biginteger,owner);_biginteger=NULL;
					q2outputError("Failed to negate a big integer");
				}
		...
		if(mp_copy(MP_INT_POINTER(_rational->num),MP_INT_POINTER(_biginteger))!=MP_OKAY){
			FREE_BIGINTEGER(_biginteger,owner);_biginteger=NULL;
			q2outputError("Failed to copy the rational numerator");
		}
	}
...
/**
 * @brief returns the rational closest to long double \p ld
 * @details \p maxiter is the maximum number of iterations allowed in approximating \p ld
 * @param ld the long double to approximate
 * @param maxiter the maximum number of iterations used in approximating \p ld
 * @return Mrational* the rational closest to \p ld
 */
Mrational* _getLongDoubleRational(long double ld,int maxiter){Mallocationowner owner=getOwner(__LINE__);
	...
	if(isLongDoubleUndefined(ld)!=M_TRUE){ // not an undefined long double (might still be Inf though)
		...
		if(ldSign!=M_ZERO){ // not zero
			...
			if(_numerator!=NULL){_rational=owned_rational(_getRational(_numerator,NULL,M_LD_NAN,false),owner);FREE_BIGINTEGER(_numerator,owner);}
		}
	}
...
/**
 * @brief returns the long double equivalent of \p _rational
 * 
 * @param _rational 
 * @return long double the long double equivalent of \p _rational
 */
long double getRationalLongDouble(Mrational const * const _rational){
	...
	if(_rational!=NULL){
		...
		if(isLongDoubleUndefined(ldNumerator)==M_FALSE){ // not undefined i.e. supposedly defined (although it could still be infinity theoretically)
			...
			if(_rational->den==NULL){if(_rational->delta!=NULL)ldNumerator+=_rational->delta->ld;return ldNumerator;}
			...
			if(isLongDoubleUndefined(ldDenominator)!=M_TRUE&&isLongDoubleZero(ldDenominator)!=M_TRUE){
				...
				q2outputError("Failed to convert a rational denominator to a long double");
		}else
			q2outputError("Failed to convert a rational numerator to a long double");
	}
	return result;
}/* VALIDATED */
...
/**
 * @brief returns the long double numerator of the unpure rational ( \p numerator / \p denominator ) + \p ld
 * 
 * @param numerator 
 * @param denominator 
 * @param ld 
 * @return long double equals \p numerator + \p ld * \p denominator
 */
long double getUnpureRationalNumerator(Mbiginteger* numerator,Mbiginteger* denominator,long double ld){
	if(numerator!=NULL){
		if(ld!=M_LD_NAN){ // as it should be
			...
			if(denominator!=NULL){ // the denominator isn't 1
				long double ldDenominator=mp_get_long_double(denominator);
				if(isLongDoubleUndefined(ldDenominator)==M_TRUE){q2outputError("Failed to convert a rational denominator to a long double.");return M_LD_NAN;}
			...
			if(isLongDoubleUndefined(ldNumerator)==M_TRUE){q2outputError("Failed to convert a rational numerator to a long double.");return M_LD_NAN;}
...
/**
 * @brief returns M_TRUE if \p rational is undefined, M_FALSE if \p rational is not undefined
 * @details at least the numerator or the denominator needs to be defined in order to be considered defined
 * @param rational 
 * @return long long M_TRUE when the rational is undefined, M_FALSE when the rational is not undefined, M_LL_INVALID if rational is NULL
 */
long long isRationalUndefined(Mrational const * const rational){return(rational!=NULL?(rational->num!=NULL||rational->den!=NULL?M_FALSE:M_TRUE):M_LL_INVALID);}
...
/**
 * @brief returns M_POSITIVE if \p rational is positive, M_NEGATIVE if \p rational is negative, M_ZERO if \p is zero, or M_LL_INVALID if \p rational is undefined
 * 
 * @param rational 
 * @return long long 
 */
long long getRationalSign(Mrational const * const rational){
	...
	if(isRationalUndefined(rational)==M_FALSE){ // the rational is defined (so it is not NULL and has a non NULL numerator and denominator)
...
/**
 * @brief returns M_TRUE if \p rational is positive, M_FALSE when \p rational is not positive, but M_LL_INVALID when rational is not defined
 * 
 * @param rational 
 * @return long long 
 */
...
/**
 * @brief returns M_TRUE if \p rational is negative, M_FALSE when \p rational is not negative, but M_LL_INVALID when rational is not defined
 * 
 * @param rational 
 * @return long long 
...
/**
 * @brief returns M_TRUE if \p rational is zero, M_FALSE when \p rational is not zero, but M_LL_INVALID when rational is not defined
 * 
 * @param rational 
 * @return long long 
 */
...
/**
 * @brief returns M_TRUE when \p rational equals 1, M_FALSE when \p rational does not equal 1, but M_LL_INVALID when \p rational is not defined
 * 
 * @param rational 
 * @return long long 
 */

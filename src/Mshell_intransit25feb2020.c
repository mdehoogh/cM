					...
					if(pNewToken->expr){
						if(pNewToken->expr->type==TT_FUNCTION_CALL){ // a function call argument
							...
							if(pNewToken->expr->argument>0){
								...
								//                what we would do on ) 
								// MDH@26JAN2021: do() and the other special functions that create their own execution environment with local variables defined in the first argument (as a map)
								//                ended up in the list of local variables
								//                however, let's also except an identifier with value equal to a map
								if(pNewToken->argument==0){
									if(prevToken->type==TT_VARIABLE){ // the list element separator (,) is right behind an identifier (TODO we should also allow array elements)
										char* variableName=_getSignificantTokenCharacters(prevToken);
										if(variableName){
											Mvalue* value=getValue(NULL,_variableName);
											if(value&&value->type==VT_MAP){
												Mmap* map=value->value._map;
												if(map){
													Mmapelement* mapelement=map->_first;
													while(mapelement){
														push_localvariable(mapelement->_variable->_name->chars)); // TODO how should we react when False is returned?
									...
									// now we can check for local variables that we need to accept as variable names that can be used in the subsequent arguments of the function call

...
size_t getUserInputLength(){
	if(inputMode==IM_COMMAND)return(_userInputCommand!=NULL&&_userInputCommand->_lastToken!=NULL?_userInputCommand->_lastToken->offset+string_length(_userInputCommand->_lastToken->text):0);

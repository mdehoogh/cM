...
Mchars* __chars(size_t size,long long count,char type){int32_t foid=getOwnerId(1);
    ...
    return DISOWNED(REALLOC(NULL,0,count,size,type,foid),foid);
}
...
Mchars* _resized(Mchars const * const _chars,size_t size,long long from_count,long long to_count,char type){int32_t foid=getOwnerId(2);
    ...
    return(DISOWNED(REALLOC(OWNED(_chars,foid),from_count,to_count,size,type,foid),foid));
}
Mchars* free_chars(Mchars const * const _chars,size_t size,long long count,char type){int32_t foid=getOwnerId(3);
    ...
    return REALLOC(OWNED(_chars,foid),count,0,size,type,foid); // obtain ownership and free
}
...
Mchars* _getChars(char const * const chars){int32_t foid=getOwnerId(4);
    Mchars* _chars=NULL;
    if(chars){
        long long l=strlen(chars)+1;
        // output("Allocating %zd characters for storing '%s'.\n",l,chars);
        _chars=(chars?OWNED(__chars(1,l,'\''),foid):NULL); // MDH@20MAY2020: obtain ownership of what _chars returns
        if(_chars){
...
void freeChars(Mchars const * const _chars){int32_t foid=getOwnerId(5);
    // if oid is not positive, assuming I was the owner to start with and use that as owner id
    free_chars(DISOWNED(OWNED(_chars,foid),foid),1,strlen(_chars->chars)+1,'\'');
}

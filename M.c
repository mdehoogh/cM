...
Mtoken* newCommandToken(TokenType tokenType,bool endOfInput){
	...
	if(endOfInput)deleteIdentifierContinuation(); // remove whatever feed forward text that was associated with the now finished last command token as it will no longer be applicabld
	...
	while(1){ // command loop
		...
		*/
		...
		while(1){ // command input loop
			...
			updateIdentifierContinuation();deleteFeedforwardText();showSuggestedText();

...
char getFirstFeedforwardCharacter(){
	// most of time _firstTokenfeedforwardtext will contain that first feed forward character
	Mtokenfeedforwardtext *tokenfeedforwardtext=_firstTokenfeedforwardtext;
	while(tokenfeedforwardtext){
		if(tokenfeedforwardtext->_text&&tokenfeedforwardtext->_text[0]!='\0')return tokenfeedforwardtext->_text[0];
...
Mtoken* newCommandToken(TokenType tokenType,bool endOfInput){
	////////deleteFeedforwardTextOfToken(pLastCommandToEvaluateToken); // remove whatever feed forward text that was associated with the now finished last command token as it will no longer be applicabld
...
//                the suggestedCharacter flag tells commandCharacterAccepted() that the input character came from feedforwardText (the feed forward), so it will in that case not alter feedforwardText (by removing the same character that was entered)
							...
							if(inputCharRead(&inputChar)){//inputChar=getInputChar();
								...
								if(inputChar==67){ // right arrow
									...
									if(_identifierContinuationText||_firstTokenfeedforwardtext){  ///// MDH@23SEP2019: replacing: behindCursor()){
										// MDH@23SEP2019: can't use feedforwardText here directly anymore after replacing behindCursor() as I've changed the feed forward 'bookkeeping' to using feed forward texts

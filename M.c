...
Mtokenfeedforwardtext* _firstTokenfeedforwardtext=NULL;
		...
		if(p){
			if(p[0]){
				...
				if(_leftover&&strlen(_leftover)>0){ // replace the feed forward text
					...
					if(prevtokenfeedforwardtext)/* not first one changed*/prevtokenfeedforwardtext->_next=nexttokenfeedforwardtext;else /* first one changed */_firstTokenfeedforwardtext=nexttokenfeedforwardtext;
				}
			}else

...
char getFirstSuggestedCharacter(bool autogenerated){return(_identifierContinuationText?getFirstIdentifierContinuationCharacter():getFirstFeedforwardCharacter(autogenerated));}
...
char consumeFirstSuggestedCharacter(char firstSuggestedCharacter,char onSuccessCharacter){
	...
	if(firstSuggestedCharacter){
		...
		// pretend to not be end of input although we are because we want to delete the accepted suggested character BEFORE updating the feed forward text
		// unless we decide to let commandCharacterAccepted take care of removing the suggested character which might actually be a good idea
		if(commandCharacterAccepted(firstSuggestedCharacter,INPUTCHARACTERTYPES[firstSuggestedCharacter],true,true))return onSuccessCharacter;
		return switchToControlMode("Failed to accept the first suggested character.");
	}
	return switchToControlMode("Failed to obtain the first suggested character!");
}
			...
			if(inputMode==IM_COMMAND){
				...
				if(inputCharType=='m'){ // Esc character...
							...
							if(inputCharRead(&inputChar)){//inputChar=getInputChar();
								...
								if(inputChar==67){ // right arrow
									...
									if(_identifierContinuationText||_firstTokenfeedforwardtext)  ///// MDH@23SEP2019: replacing: behindCursor()){
										inputCharType=consumeFirstSuggestedCharacter('\0',inputCharType);
									else
										beep();
								}else
					...
					if(inputChar==getFirstSuggestedCharacter(true))
						inputCharType=consumeFirstSuggestedCharacter(inputChar,inputCharType);
					else

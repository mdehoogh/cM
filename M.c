...
void updateIdentifierContinuation(){
	...
	if(!couldHaveAnIdentifierContinuation)identifierCouldHaveChanged=false; // if we're not in an identifier token always consider the identifier to be unchanged
	if(!couldHaveAnIdentifierContinuation||identifierCouldHaveChanged)deleteIdentifierContinuation();
	if(identifierCouldHaveChanged){ // the identifier has supposedly changed, and we could have an identifier continuation update it
...
// we can keep using feedforwardText but instead we need to compose it each time it is requested
// call deleteTokenfeedforwardtexts whenever the behind cursor text changes
typedef struct Mtokenfeedforwardtext{
	Mtoken* token;
	char* _text;
	struct Mtokenfeedforwardtext* _next;
...
}Mtokenfeedforwardtext;
// MDH@30SEP2019: all token feed forward texts accepted (i.e. consumed) are pointed to by _lastConsumedTokenfeedforwardtext
...
Mtokenfeedforwardtext *_firstTokenfeedforwardtext=NULL,*_lastConsumedTokenfeedforwardtext=NULL;
...
Mstring* _getFeedforwardText(char sep){
	// constructs the total feed forward text
	Mstring* _feedforwardText=__string();
	if(_feedforwardText){
		...
		if(!sep){
			if(_identifierContinuationText){
				...
				if(numberOfIdentifierContinuationCharacters>0){
					size_t numberOfFeedforwardCharacters=string_length(_feedforwardText);
					if(numberOfIdentifierContinuationCharacters<numberOfFeedforwardCharacters){ // there might be feed forward characters left
						char c=string_replacedchar(_feedforwardText,'\0',numberOfIdentifierContinuationCharacters); // temporarily pretend the feed forward text to have the same length as the identifier continuation text
						bool matching=(strcmp(string(_feedforwardText),_identifierContinuationText)==0); // get the comparison result
						string_setchar(_feedforwardText,c,numberOfIdentifierContinuationCharacters); // // put the removed character back BEFORE removing the identifier continuation when matching
						if(matching) // the same!!!
							if(string_removed(_feedforwardText,0,numberOfIdentifierContinuationCharacters)!=numberOfIdentifierContinuationCharacters) // remove the identifier continuation from the feed forward text
								inputError("Failed to remove the identifier continuation (completely) from the start of the rest of the suggested text");
					}else
					if(numberOfIdentifierContinuationCharacters==numberOfIdentifierContinuationCharacters) // 'remove' all feed forward characters
						string_setlength(_feedforwardText,0);
				}
			}
		}
	}
	return _feedforwardText;
}
...
char* getFeedforwardCharacters(){
	deleteFeedforwardText(); // forces update TODO can be removed iff on every change to the list of feed forward text deleteFeedforwardText() is called!!! (so this is JIT deletion)
	...
	int result=feedforwardTextLength; // remember the current length
	feedforwardText=_getFeedforwardCharacters('\0');
	// update the length of the behind cursor text length
	feedforwardTextLength=(feedforwardText?string_length(feedforwardText):0);
...
void deleteTokenfeedforwardtexts(){
...
void setLastTokenFeedforwardText(char* _text){
	...
	if(lastTokenFeedforwardText){ // yes, so replace the text contents
		...
		if(strlen(_text)>0){
			if(amDebugging())inputInfo("Prepending feed forward text '%s' of token '%s' with offset %zu.",_text,string(pLastCommandToEvaluateToken->text),pLastCommandToEvaluateToken->offset);
			...
			if(_tokenfeedforwardtext){
				...
				if(amDebugging())inputInfo("Feed forward text '%s' of token '%s' prepended!",_text,string(pLastCommandToEvaluateToken->text));
			}else
...
//                NOTE deleteSuggestedTextOfToken delegates to deleteFeedforwardTextOfToken
bool deleteFeedforwardTextOfToken(Mtoken* token){
	...
	if(!tokenfeedforwardtext)return false; // the token apparently does not have feed forward text
...
void removeFirstFeedforwardCharacterFromLastTokenWhenMatching(char inputChar){
	...
	Mtokenfeedforwardtext* _feedforwardtext=_firstTokenfeedforwardtext;while(_feedforwardtext&&_feedforwardtext->token!=pLastCommandToEvaluateToken)_feedforwardtext=_feedforwardtext->_next;
	if(!_feedforwardtext)return;
...
//                so, replacing getFirstFeedforwardCharacterRemoved() by getFirstFeedforwardCharacter() and a function to actually delete that first feed forward character (and either consume or delete it)
char getFirstFeedforwardCharacter(bool autogenerated){
	...
	// most of time _firstTokenfeedforwardtext will contain that first feed forward character
	Mtokenfeedforwardtext *tokenfeedforwardtext=_firstTokenfeedforwardtext;
	while(tokenfeedforwardtext){
		if(tokenfeedforwardtext->_text&&tokenfeedforwardtext->_text[0]!='\0')return(!autogenerated||tokenfeedforwardtext->token?tokenfeedforwardtext->_text[0]:'\0');
		tokenfeedforwardtext=tokenfeedforwardtext->_next; // get the next to check
	}
	return '\0';
}
bool deleteFirstFeedforwardCharacter(char firstFeedforwardCharacter,bool consumed){
	if(firstFeedforwardCharacter!='\0'){
		// find it
		Mtokenfeedforwardtext *prevtokenfeedforwardtext=NULL,*tokenfeedforwardtext=_firstTokenfeedforwardtext;
		while(tokenfeedforwardtext){
			if(tokenfeedforwardtext->_text&&tokenfeedforwardtext->_text[0]==firstFeedforwardCharacter)break;
			prevtokenfeedforwardtext=tokenfeedforwardtext; // remember the current feed forward text as the previous feed forward text
			tokenfeedforwardtext=prevtokenfeedforwardtext->_next; // get the next to check
		}
		if(tokenfeedforwardtext){ // got it
			...
			if(strlen(tokenfeedforwardtext->_text)==1){
				if(consumed){ // i.e. we should remember the token text, so we can unconsume it
					Mtokenfeedforwardtext* nextFirstTokenfeedforwardtext=NULL;
					while(_firstTokenfeedforwardtext){
						nextFirstTokenfeedforwardtext=_firstTokenfeedforwardtext->_next; // remember what the next first token feed forward text will become
						// make the current first token feed forward text point to what the current last consumed token feed forward text is
						_firstTokenfeedforwardtext->_next=_lastConsumedTokenfeedforwardtext;
						// set the last consumed token feed forward text to the current first token feed forward text
						_lastConsumedTokenfeedforwardtext=_firstTokenfeedforwardtext;
						// update the first token feed forward text to what it was originally pointing to
						_firstTokenfeedforwardtext=nextFirstTokenfeedforwardtext;
						// if the token feed forward text with the first feed forward character was consumed, we're done
						if(_lastConsumedTokenfeedforwardtext==tokenfeedforwardtext)break;
					}
				}else{ // a true delete
					// replace the current text by what's behind the first character (if any)
					char* p=tokenfeedforwardtext->_text;
					...
					if(!tokenfeedforwardtext->_text){ // nothing left (or failing to copy the remainder over)
						...
						if(prevtokenfeedforwardtext)prevtokenfeedforwardtext->_next=nexttokenfeedforwardtext;else _firstTokenfeedforwardtext=nexttokenfeedforwardtext;
					}
				}
			}else{
...
char getFirstFeedforwardCharacterRemoved(){
	...
	Mtokenfeedforwardtext *prevtokenfeedforwardtext=NULL,*tokenfeedforwardtext=_firstTokenfeedforwardtext;
	while(tokenfeedforwardtext&&(!tokenfeedforwardtext->_text||tokenfeedforwardtext->_text[0]=='\0')){prevtokenfeedforwardtext=tokenfeedforwardtext;tokenfeedforwardtext=prevtokenfeedforwardtext->_next;}
	if(tokenfeedforwardtext){
		char* p=tokenfeedforwardtext->_text;
		if(p){
			if(p[0]){
	...
	return firstFeedforwardCharacterRemoved;
}
...
Mtokenfeedforwardtext*  getFeedforwardTextOfCharacterPrepended(char c,bool new){
	Mtokenfeedforwardtext* result=NULL;
	if(c){
		...
		if(!new&&_lastConsumedTokenfeedforwardtext&&_lastConsumedTokenfeedforwardtext->_text&&_lastConsumedTokenfeedforwardtext->_text[0]==c){
			...
			result=_lastConsumedTokenfeedforwardtext;
		}else{
			...
			if(feedforwardtext){
				...
				if(_string){
					...
					*/
					...
					result=feedforwardtext; // remember the 
					deleteFeedforwardText();
				}
				// if failed, ascertain to free what is not bound in the feedforward text list
				if(!result){feedforwardtext->_next=NULL;free_tokenfeedforwardtext(feedforwardtext);feedforwardtext=NULL;}
			}
		}
	}
	return result;
...
//                deleteLastTokenImmediateFeedforwardText() is to be called when pLastCommandToEvaluateToken stops being the current token or when the type of the current token changes
//                addLastTokenImmediateFeedforwardText() is to be called when pLastCommandToEvaluateToken just became the current token (or when its type changes)
/**
 * \brief removes the immediate feed forward character of the current token (if any)
 */ 
bool deleteLastTokenImmediateFeedforwardText(){
	// we only delete the associated feed forward token of there's an immediate feed forward character
	return(!getImmediateFeedforwardCharacterOfToken(pLastCommandToEvaluateToken)||deleteFeedforwardTextOfToken(pLastCommandToEvaluateToken));
}
/**
 * \brief prepends the immediate feed forward character of the current token (if any), returns true on success, false otherwise
 */
bool addLastTokenImmediateFeedforwardText(){
	// MDH@03OCT2019: getFeedforwardTextOfCharacterPrepended was adjusted to return true when the character passed to it equals '\0'!!
	...
	char lastTokenImmediateFeedforwardCharacter=getImmediateFeedforwardCharacterOfToken(pLastCommandToEvaluateToken);
	if(!lastTokenImmediateFeedforwardCharacter)return true;
	// try to prepend the character
	Mtokenfeedforwardtext* lastTokenImmediateFeedforwardtext=getFeedforwardTextOfCharacterPrepended(lastTokenImmediateFeedforwardCharacter,true); // second argument forces always prepending this character!!
	if(!lastTokenImmediateFeedforwardtext)return false; // failed to prepend the immediate feed forward text anonymously
	lastTokenImmediateFeedforwardtext->token=pLastCommandToEvaluateToken; // remember the associated token!!!
	return true;
}
...
void setLastCommandToken(Mtoken* newLastCommandToken){
	...
	if(!deleteLastTokenImmediateFeedforwardText())inputError("Failed to remove the last token immediate feed forward text.");
	pLastCommandToEvaluateToken=newLastCommandToken;
	if(!addLastTokenImmediateFeedforwardText())inputError("Failed to add the last token immediate feed forward text.");
	identifierCouldHaveChanged=inIdentifierToken();
}


void deleteSuggestedTextOfToken(Mtoken* token,bool deleteIdentifierContinuationText){
	if(!token)return;
	if(deleteIdentifierContinuationText)deleteIdentifierContinuation(); // this is the easiest way but makes sense
	deleteFeedforwardTextOfToken(token);
}
...
size_t cursorPosition(){return(inputMode==IM_COMMAND?(pLastCommandToEvaluateToken?pLastCommandToEvaluateToken->offset+string_length(pLastCommandToEvaluateToken->text):0):(inputMode==IM_SHELL?string_length(shellCommand):0));}
size_t behindCursor(){return strlen(getFeedforwardCharacters());} // MDH@20SEP2019: feedforwardText replaced by getFeedforwardCharacters(), this is done everywhere!!!!!
size_t getCommandLength(){return cursorPosition()+behindCursor();}
...
void outputDebugInfo(){
	...
	Mstring* _separatedBehindCursorText=_getFeedforwardText('|');
	/////////debugWrite("Status: Cursor position=%u - command length=%u - behind cursor text='%s'.",cursorPosition(),getCommandLength(),string(feedforwardText));
	inputInfo("Cursor position: %zu | Command length: %zu | Identifier continuation: '%s' | Feed forward: '%s'.",cursorPosition(),getCommandLength(),(_identifierContinuationText?_identifierContinuationText:"-"),string(_separatedBehindCursorText));
	free_string(_separatedBehindCursorText);
	//////outputInfo("Status: Cursor position=%u - command length=%u - behind cursor text='%s'.",cursorPosition(),getCommandLength(),string(feedforwardText));
}
...
bool removeToken(){		
	...
	deleteSuggestedTextOfToken(pLastCommandToEvaluateToken,true);
	...
	// MDH@02OCT2019: every time pLastCommandToEvaluateToken changes, call setLastCommandToken which will set identifierCouldHaveChanged if it's an identifier token
	...
	if(inIdentifierToken())identifierCouldHaveChanged=true; // every time the last command to evaluate token changes, we need this
	*/
...
void writeCommand(){
	...
	identifierCouldHaveChanged=inIdentifierToken(); // MDH@02OCT2019 because we're setting pLastCommandToEvaluateToken but not calling setLastCommandToken()
}
...
void setInputMode(enum INPUTMODE_ENUM newInputMode){
	...
	if(inputMode!=IM_COMMAND)feedforwardText=__string();
}
...
size_t getNumberOfFeedforwardCharactersWritten(){
	...
	size_t feedforwardCharactersLength=(feedforwardCharacters?strlen(feedforwardCharacters):0);
	if(feedforwardCharactersLength){ // additional feed forward text to write
		debugWrite("Feed forward characters to write: '%s'.",feedforwardCharacters);
		setColor(getFeedForwardTextColor());
		output("%s",feedforwardCharacters);
	}
...
void hideSuggestedText(){
	...
	outputChar(' ');
	moveCursorLeft(numberOfSuggestedCharactersWritten+1);
}
...
void setCommandIndex(uint32_t newCommandIndex){
	...
	deleteFeedforwardText(); // MDH@20SEP2019 replacing: string_setlength(feedforwardText,0); 
	if(commandIndex){
...
void setLastTokenType(TokenType tokenType,bool endOfInput){
	if(pLastCommandToEvaluateToken){
		if(tokenType!=pLastCommandToEvaluateToken->type){
			if(!deleteLastTokenImmediateFeedforwardText())inputError("Failed to remove the current token immediate feed forward text.");
			pLastCommandToEvaluateToken->type=tokenType;
			if(!addLastTokenImmediateFeedforwardText())inputError("Failed to add the current token immediate feed forward text.");
		}
	}
...
Mtoken* newCommandToken(TokenType tokenType,bool endOfInput){
	...
	identifierCouldHaveChanged=inIdentifierToken(); // MDH@02OCT2019: as we're setting the type of the token AFTER creating it, we wait until after doing so to update identifierCouldHaveChanged!!
...
void newCommand(){
	...
	pLastCommandToEvaluateToken=NULL;pCommandToEvaluate=newCommandToken(TT_EXPRESSION,true);if(!pCommandToEvaluate)outputError("Failed to create a new command");else pCommandToEvaluate->expr=NULL;
	/* replacing:
	...
	if(pLastCommandToEvaluateToken->type!=TT_FUNCTION){ // is it a function (now)?
		if(getFunction(getEnvironment(),_identifierName)){ // yes, it is
			...
			*/
			...
			// MDH@23SEP2019 take care of by setLastTokenType, so removed: if(endOfInput&&amMatchingparentheses())setLastTokenFeedforwardText("(");else deleteFeedforwardTextOfToken(pLastCommandToEvaluateToken); // MDH@20SEP2019: either force the feedforward text to match an opening parenthesis or nothing TODO does endOfInput matter?????
		...
		if(!getFunction(getEnvironment(),_identifierName)){ // no, it ain't
			...
			// MDH@23SEP2019 take care of by setLastTokenType, so removed: deleteFeedforwardTextOfToken(pLastCommandToEvaluateToken); // MDH@20SEP2019: I suppose when TT_VARIABLE changes to TT_NEW_VARIABLE later on, an equal sign might be added!!!
			/* MDH@20SEP2019 replacing:
	...
	if(pLastCommandToEvaluateToken->type==TT_VARIABLE||pLastCommandToEvaluateToken->type==TT_NEW_VARIABLE){ // might not exist after all both in the command and in the current environment
		...
		if(pLastCommandToEvaluateToken->type==TT_VARIABLE){ // might not exist after all both in the command and in the current environment
			if(!variableExists){ // apparently does NOT exist
				...
				// MDH@23SEP2019 take care of by setLastTokenType, so removed: setLastTokenFeedforwardText("="); // MDH@20SEP2019 replacing: if(endOfInput&&!aSuggestedCharacter)if(amMatchingparentheses())if(string_char(feedforwardText,0)!='=')string_insert_char(feedforwardText,0,'=');
			}
		}else{ // a new variable
			if(variableExists){ // now an existing variable
				setLastTokenType(TT_VARIABLE,endOfInput);
				reoutputToken(pLastCommandToEvaluateToken);
				///// MDH@23SEP2019 removed: deleteFeedforwardTextOfToken(pLastCommandToEvaluateToken); // MDH@20SEP2019 replacing: if(endOfInput)if(amMatchingparentheses())if(string_length(feedforwardText)&&string_char(feedforwardText,0)=='=')string_removed_char(feedforwardText,0);
			}
		}
	}
...
void cancelCommand(){ // in response to Ctrl-C or backspace on the first character
	...
	deleteTokenfeedforwardtexts(); // MDH@20SEP2019 replacing: string_setlength(feedforwardText,0); 
...
#endif	
		identifierCouldHaveChanged=inIdentifierToken(); // MDH@02OCT2019: should be called whenever pLastCommandToEvaluateToken changes...
					...
					if(pLastCommandToEvaluateToken->type!=TT_BINARY_aErU){
						...
						if(amMatchingparentheses()){
							...
							switch(inputCharacterType){
								case '[':
									setLastTokenFeedforwardText("]"); // MDH@20SEP2019 replacing: string_insert_char(feedforwardText,0,']');
									break;
								case '{':
									setLastTokenFeedforwardText("}"); // MDH@20SEP2019 replacing: string_insert_char(feedforwardText,0,'}');
									break;
								...
								case 'D':
									...
									setLastTokenFeedforwardText(string(pLastCommandToEvaluateToken->text)); // MDH@20SEP2019 replacing: string_insert_char(feedforwardText,0,inputChar);
									break;
								case 'S':
...
void clearShellCommand(){
	string_setlength(feedforwardText,0);
...
char getFirstSuggestedCharacter(bool autogenerated){return(_identifierContinuationText&&strlen(_identifierContinuationText)>0?getFirstIdentifierContinuationCharacter():getFirstFeedforwardCharacter(autogenerated));}
...
void removeFirstSuggestedCharacter(char firstSuggestedCharacter,bool consumed){
	if(!_identifierContinuationText||strlen(_identifierContinuationText)==0){ // a feed forward text character was consumed
		deleteFirstFeedforwardCharacter(firstSuggestedCharacter,consumed);
	}
}
	...
	while(1){ // command loop
		...
		*/
		...
		if(inputMode==IM_COMMAND){
			...
			getCommandLength()=cursorPosition()=writeTokens(pCommandToEvaluate);
			*/
			/////////outputStatus();
		}
		...
		*/
		...
		while(1){ // command input loop
			...
			if(inputMode==IM_COMMAND){
				...
				deleteFeedforwardText(); // to force it being reconstructed!!! TODO if we decide to always do that we do not need to do this here!!!
				///outputChar('C');
				showSuggestedText();
				///outputChar('D');
			...
			if(inputMode==IM_COMMAND){
				...
				if(inputCharType=='b'){ // backspace
					...
					// something to remove?
					if(cursorPosition()) // TODO pCommandToEvaluate should be NULL at the same time getCommandLength() becomes 0!!!
				...
				if(inputCharType=='t'){ // Tab character
					...
					/* replacing:
					if(_firstTokenfeedforwardtext){
						char newInputChar;
						while((newInputChar=getFirstFeedforwardCharacterRemoved())){
					...
					*/
					...
					char* consumableText=(_identifierContinuationText&&strlen(_identifierContinuationText)>0?_identifierContinuationText:getFeedforwardCharacters());
					...
					if(numberOfConsumableCharacters){
						...
						if(_identifierContinuationText&&strlen(_identifierContinuationText)>0)deleteIdentifierContinuation();else deleteTokenfeedforwardtexts();
						// if successful
						if(newInputChar){
						...
						if(inputChar==91){
							if(inputCharRead(&inputChar)){//inputChar=getInputChar();
								...
								if(inputChar==51){
									if(inputCharRead(&inputChar)){//inputChar=getInputChar();
										if(inputChar==126){ // delete
											...
											if(_identifierContinuationText||_firstTokenfeedforwardtext){
												if(_identifierContinuationText){
													if(strlen(_identifierContinuationText)>0){
														...
														//// MDH@02OCT2019: the = sign would already be there: updateLastTokenFeedforwardText(false); // we do this because deleting identifier continuation might possibly force an assignment operator to appear behind a new variable (otherwise it wouldn't)
								...
								if(inputChar==67){ // right arrow
									...
									if(_identifierContinuationText||_firstTokenfeedforwardtext){  ///// MDH@23SEP2019: replacing: behindCursor()){
										...
										if(c){ // removing the character behind the cursor succeeded
											...
											*/
											/*
												...
												removeToken();
											*/
											// MDH@30SEP2019: this is the only call to getFeedforwardTextOfCharacterPrepended() therefore we can simply adjust that function to check whether this character matches a consumed character!!!
											...
											if(!getFeedforwardTextOfCharacterPrepended(c,false))
											...
											if(pLastCommandToEvaluateToken->type==TT_FUNCTION||pLastCommandToEvaluateToken->type==TT_VARIABLE||pLastCommandToEvaluateToken->type==TT_NEW_VARIABLE){
											...
											if(!getFeedforwardTextOfCharacterPrepended(c))
											inputError("Failed to accept the removed command character as suggested text.");

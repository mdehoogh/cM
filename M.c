...
char* _getLastTokenAutoCompletionText(){
	...
	switch(pLastCommandToEvaluateToken->type){
		...
		case TT_ERROR:break;
		case TT_EXPRESSION:if(string_last_char(pLastCommandToEvaluateToken->text)=='(')tokenAutoCompletionText=")";break; // TODO use other type e.g. TT_FUNCTION_CALL instead of TT_EXPRESSION on (
...
void setLastTokenAutocompletionText(char* _text){
	...
	Mtokenautocompletiontext* lastTokenAutocompletionText=getTokenAutocompletionText(pLastCommandToEvaluateToken);
	if(lastTokenAutocompletionText){ // yes, so replace the text contents
		...
		if(strlen(_text)>0){
			if(amDebugging())inputInfo("Prepending auto completion text '%s' of token '%s' with offset %zu.",_text,string(pLastCommandToEvaluateToken->text),pLastCommandToEvaluateToken->offset);
			...
			if(_tokenautocompletiontext){
				...
				_tokenautocompletiontext->token=pLastCommandToEvaluateToken;
...
void removeFirstFeedforwardCharacterFromLastTokenWhenMatching(char inputChar){
	...
	Mtokenautocompletiontext* _autocompletiontext=_firstTokenautocompletiontext;while(_autocompletiontext&&_autocompletiontext->token!=pLastCommandToEvaluateToken)_autocompletiontext=_autocompletiontext->_next;
	if(!_autocompletiontext)return;
...
//                updateImmediateFeedforwardText() is to be called when pLastCommandToEvaluateToken just became the current token (or when its type changes)
...
bool updateImmediateFeedforwardText(){
	...
	immediateFeedforwardToken=pLastCommandToEvaluateToken;
...
void showSuggestedText(){
	...
	resetOutputColor();
	if(_suggestedText)
	// 1. write the identifier continuation text first, then the immediate feed forward text, and finally the more or less permanent feed forward texts
	numberOfSuggestedCharactersWritten=getNumberOfIdentifierContinuationTextCharactersWritten()+getNumberOfImmediateFeedforwardCharactersWritten()+getNumberOfAutocompletionCharactersWritten();
						...
						if(amMatchingparentheses()){
							...
							switch(inputCharacterType){
								...
								case 'D':
									...
									setLastTokenAutocompletionText(string(pLastCommandToEvaluateToken->text)); // MDH@20SEP2019 replacing: string_insert_char(feedforwardText,0,inputChar);
									break;
								case 'S':
	...
	while(1){ // command loop
		...
		*/
		...
		if(inputMode==IM_COMMAND){
			...
			if(pCommandToEvaluate){writeCommand();/*writeSuggestedText(true);*/}else deleteTokenautocompletiontexts(); // MDH@20SEP2019 replacing: string_setlength(feedforwardText,0);
		...
		*/
		...
		while(1){ // command input loop
			...
			if(inputMode==IM_COMMAND){
				// update all text suggestions: the identifier continuation, the immediate feed forward text and the accumulative feed forward text
				// NOTE if the feed forward text starts with the indentifier continuation text the identifier continuation text should be removed from it!!!!
				updateIdentifierContinuation();
				// MDH@04OCT2019: it's more reliable to add the immediate feed forward text here, and remove it immediately after receiving the input character
				if(immediateFeedforwardToBeUpdated)updateImmediateFeedforwardText();else immediateFeedforwardToBeUpdated=true; // MDH@04OCT2019: now also takes care of removing any previous feed forward text
				///outputChar('B');
			...
			if(inputMode==IM_COMMAND){
				...
				if(inputCharType=='d'){ // MDH@18APR2019: delete now always deletes the first character in the behind cursor text
					...
					if(_firstTokenautocompletiontext){ // replacing: string_length(feedforwardText)){ // something to delete
						if(getFirstAutocompletionCharacterRemoved()){ // replacing: string_removed_char(feedforwardText,0)){ // success!!!
							// MDH@25SEP2019: now done in getFirstAutocompletionCharacterRemoved() removing: deleteTokenautocompletiontexts();
							////////writeSuggestedText(false);
						}else
									...
									if(cursorPosition()){
										...
										if(c){ // removing the character behind the cursor succeeded
											...
											*/
											...
											if(!getAutocompletionTextOfCharacterPrepended(c,true))
